...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        ...
        words_embeddings = self.word_embeddings(input_ids.clone())
...
class LanguageEncoder(nn.Module):
    def __init__(self, config):
        ...
        if self.config.do_back_txt or self.config.do_front_txt:
            ...
            self.z_concat_layernorm = BertLayerNorm(config.hidden_size, eps=1e-12)
            self.z_direct_ln = BertLayerNorm(config.hidden_size, eps=1e-12)
            self.z_landm_ln = BertLayerNorm(config.hidden_size, eps=1e-12)
            # type_2
            self.z_direc_cross_attn = BertXAttention(config)
            self.z_landm_cross_attn = BertXAttention(config)
        ...
        if config.do_front_txt:
            self.z_front_cross_attn = BertXAttention(config)
            self.z_front_linear = nn.Linear(config.hidden_size,config.hidden_size)
            self.z_front_ln = BertLayerNorm(config.hidden_size, eps=1e-12)

    def forward(self, txt_embeds, txt_masks,
                instr_z_direction_features=None, instr_z_direction_pzs=None, instr_z_landmark_features=None, instr_z_landmark_pzs=None,
                front_txt_embeds=None):
        extended_txt_masks = extend_neg_masks(txt_masks)
        for layer_module in self.layer:
            temp_output = layer_module(txt_embeds, extended_txt_masks)
            txt_embeds = temp_output[0]
        if not self.update_lang_bert: # default: True
        ...
        if self.config.do_back_txt or self.config.do_front_txt:
            if self.config.do_back_txt:
                z_direc_embeds = self.z_direc_cross_attn(txt_embeds, 
                                                        ctx_tensor=instr_z_direction_features)[0] 
                z_direc_embeds = self.z_direct_ln(self.z_direct_linear(z_direc_embeds))
                if instr_z_landmark_features is not None:
                    z_landm_embeds = self.z_landm_cross_attn(txt_embeds, 
                                                            ctx_tensor=instr_z_landmark_features)[0]
            ...
            if self.config.do_add_method == 'door':
                ...
                if self.config.do_instr:
                    txt_embeds = txt_embeds + z_direc_embeds + z_landm_embeds
                if self.config.do_tim_txt and front_txt_embeds is not None:
...
class ImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        if config.do_back_img:
            ...
            if self.config.do_back_img_type == 'type_2':
                self.do_img_attn = BertXAttention(config)
    ...
    def forward(
        ...
        if z_img_features is not None:
            ...
            traj_view_img_embeds = self.img_after_linear(traj_view_img_embeds) + self.do_img_after_linear(sum_z_img)
            traj_view_img_embeds = self.do_img_concat_layernorm(traj_view_img_embeds)
...
class NextActionPrediction(nn.Module):
    def __init__(self, hidden_size, dropout_rate=0.1, add_dropout=True):
        super().__init__()
        if add_dropout:
...
class FrontDoorEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.lg_cross_attn = BertXAttention(config)
        self.ln = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.config = config

        if config.front_lg_door:
    ...
    def forward(self, local_feats, global_feats, local_feats_masks=None):
        ...
        if self.config.front_self_attn:
            ll_feats = self.ll_self_attn(local_feats, attention_mask=local_feats_masks)[0]
        else:
            ll_feats = local_feats
        lg_feats = self.lg_cross_attn(local_feats, ctx_tensor=global_feats)[0]
        out_feats = self.ln(ll_feats + lg_feats)

        if self.config.front_lg_door:
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.global_sap_head = NextActionPrediction(self.config.hidden_size, add_dropout=config.add_next_action_pred_dropout)
        
        # local branch
        if self.config.add_local_branch:
            self.local_encoder = LocalVPEncoder(config)
            self.local_sap_head = NextActionPrediction(self.config.hidden_size, add_dropout=config.add_next_action_pred_dropout)
        ...
        # causal intervention
        if config.do_front_local:
            self.front_local_encoder = FrontDoorEncoder(config)
        if config.do_front_global:
    ...
    def forward_panorama(
        ...
        if z_img_features is not None:
            ...
            # type-1
                ...
                p_z_img = z_img_embeds * z_img_pzs.to(torch.float32)
                sum_z_img = torch.sum(p_z_img,1).unsqueeze(1) #[bs,1,dim]
                rgb_embeds = self.img_embeddings.img_after_linear(rgb_embeds) + self.img_embeddings.do_img_after_linear(sum_z_img)
                rgb_embeds = self.img_embeddings.do_img_concat_layernorm(rgb_embeds)
            elif self.config.do_back_img_type == 'type_2':
                z_img_embeds = self.img_embeddings.do_img_layer_norm(self.img_embeddings.do_img_before_linear(z_img_features))     
                z_img_embeds = self.img_embeddings.do_img_attn(rgb_embeds, ctx_tensor=z_img_embeds)[0]
                if self.config.do_add_method == 'door':
                    ori_img_embeds = self.img_embeddings.img_after_linear(rgb_embeds)
                    ...
                    rgb_embeds = torch.mul(aug_img_weight, rgb_embeds) + torch.mul((1-aug_img_weight),z_img_embeds)
                elif self.config.do_add_method == 'add':
                    rgb_embeds = rgb_embeds + z_img_embeds
                elif self.config.do_add_method == 'concat':
                    concat_img_embeds = torch.cat((rgb_embeds, z_img_embeds),-1)
                    rgb_embeds = self.img_embeddings.do_concat_img_linear(concat_img_embeds)
        ...
        try:
            ...
            print(img_embeds.shape, loc_fts.shape, nav_types.shape)
            raise Exception("Error in panorama embedding")
    ...
    def forward_navigation(
        ...
        front_txt_feats=None, front_vp_feats=None, front_gmap_feats=None,
    ):
        # global branch
        gmap_embeds = gmap_img_fts + \
    ...
    def encode_vpids(self, vpids_list, node2id, device):
        # vpids_list: (batch_size, list of node names)
        batch_tensor = []
        max_len = max(len(vpids) for vpids in vpids_list) 
        for vpids in vpids_list:
            tensor_row = []
            for vp in vpids:
                if vp is None:
                    tensor_row.append(-1) 
                else:
                    tensor_row.append(node2id[vp])
            while len(tensor_row) < max_len:
                tensor_row.append(-1)  
            batch_tensor.append(tensor_row)
        return torch.tensor(batch_tensor, device=device)

    def forward_navigation_with_local_branch(
        self, txt_embeds, txt_masks, 
        gmap_vpids, gmap_step_ids, 
        gmap_img_fts, gmap_pos_fts, 
        gmap_masks, gmap_visited_masks, gmap_pair_dists,
        vp_img_embeds, vp_pos_fts, vp_masks, vp_nav_masks, vp_cand_vpids,
        front_txt_feats=None, front_vp_feats=None, front_gmap_feats=None,
    ):
        batch_size = gmap_img_fts.shape[0]
        ...
        # fusion
        fused_logits = global_logits.clone()
        fused_logits[:, 0] = fused_logits[:, 0] + local_logits[:, 0]  # stop
        
        # 1. Build node2id mapping
        all_nodes = set()
        for vpids in gmap_vpids + vp_cand_vpids:
            for vp in vpids:
                if vp is not None:
                    all_nodes.add(vp)
        ...
        gmap_vpids_tensor = self.encode_vpids(gmap_vpids, node2id, gmap_embeds.device)  # (batch_size, gmap_len)
        vp_cand_vpids_tensor = self.encode_vpids(vp_cand_vpids, node2id, gmap_embeds.device)  # (batch_size, vp_len)

        batch_size, max_gmap_len = gmap_vpids_tensor.shape
        _, max_vp_len = vp_cand_vpids_tensor.shape

        # 1. Prepare masks
        visited_nodes = gmap_visited_masks.bool()  # (B, gmap_len)

        # Expand vp_cand_vpids for comparison
        vp_cand_vpids_expand = vp_cand_vpids_tensor.unsqueeze(2).expand(-1, -1, max_gmap_len)  # (B, vp_len, gmap_len)
        gmap_vpids_expand = gmap_vpids_tensor.unsqueeze(1).expand(-1, max_vp_len, -1)          # (B, vp_len, gmap_len)

        # Mask: (B, vp_len, gmap_len) where vp_cand_vpid == gmap_vpid
        matching = (vp_cand_vpids_expand == gmap_vpids_expand)  # (B, vp_len, gmap_len)

        # visited mask per cand_vpid
        visited_expand = visited_nodes.unsqueeze(1).expand_as(matching)  # (B, vp_len, gmap_len)
        cand_is_visited = (matching & visited_expand).any(dim=2)  # (B, vp_len)

        # j_bound mask
        j_bound = 0
        valid_idx = torch.arange(max_vp_len, device=vp_cand_vpids_expand.device).unsqueeze(0) > j_bound  # (1, vp_len)

        # Only valid candidates
        valid_cands = valid_idx.expand(batch_size, -1)  # (B, vp_len)

        # 2. bw_logits: sum of local_logits over visited candidates
        real_vp_len = vp_cand_vpids_tensor.shape[1] 
        local_logits = local_logits[:, :real_vp_len]
        
        visited_logits = local_logits * cand_is_visited.float() * valid_cands.float()  # (B, vp_len)
        bw_logits = visited_logits.sum(dim=1, keepdim=True)  # (B, 1)

        # 3. tmp logits: local logits for non-visited candidates
        non_visited_logits = local_logits * (~cand_is_visited).float() * valid_cands.float()  # (B, vp_len)

        # 4. Now map non-visited local logits to fused_logits
        # For each cand_vpid, find its matching gmap_vpid
        # (B, vp_len, gmap_len)
        match_score = matching.float() * non_visited_logits.unsqueeze(2)  # (B, vp_len, gmap_len)

        # Sum contributions of different candidates to gmap points
        # (B, gmap_len)
        tmp_logits = match_score.sum(dim=1)

        # 5. Add tmp_logits + bw_logits
        fused_logits += tmp_logits + (bw_logits * visited_nodes.float())
        
        fused_logits.masked_fill_(gmap_visited_masks, -float('inf'))
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            txt_embeds = self.forward_txt(batch['txt_ids'], batch['txt_masks'])
            return txt_embeds

        elif mode == 'panorama':
            pano_embeds, pano_masks = self.forward_panorama(
                batch['view_img_fts'], batch['obj_img_fts'], batch['loc_fts'],
                batch['nav_types'], batch['view_lens'], batch['obj_lens'],
                z_img_features=batch['z_img_features'], z_img_pzs=batch['z_img_pzs']
            )
            return pano_embeds, pano_masks
        
        elif mode == 'navigation':
            return self.forward_navigation(
        ...
        elif mode == 'navigation_with_local_branch':
             return self.forward_navigation_with_local_branch(
                batch['txt_embeds'], batch['txt_masks'], batch['gmap_img_embeds'], 
                batch['gmap_step_ids'], batch['gmap_pos_fts'], batch['gmap_masks'],
                batch['gmap_pair_dists'], batch['gmap_visited_masks'], batch['gmap_vpids'], 
