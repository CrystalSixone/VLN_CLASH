...
class PolicyViewSelectionCLASH(ILPolicy):
    def __init__(
        self,
    ...
    @classmethod
    def from_config(
        cls, config, observation_space: Space, action_space: Space
    ):
        config.defrost()
...
class ETP(Net):
    def __init__(
        self, observation_space: Space, model_config, num_actions,
        config=None,
    ):
        super().__init__()
        ...
        print('\nInitalizing the ETP-CLASH model ...')
        self.vln_bert = get_vlnbert_models(config=model_config)
        
        env_dropout_prob = model_config.env_dropout_prob if hasattr(model_config, 'env_dropout_prob') else 0.4
        self.drop_env = nn.Dropout(p=env_dropout_prob)
        ...
        self.rgb_encoder = CLIPEncoder(model_config, self.device)
    ...
    def forward(self, mode=None, 
                ...
                vp_img_embeds=None, vp_pos_fts=None, vp_masks=None, vp_nav_masks=None, vp_cand_vpids=None,
                z_img_features=None, z_img_pzs=None,
                front_txt_feats=None, front_vp_feats=None, front_gmap_feats=None,
                already_dropout=False):
        
        if mode == 'language':
            if instr_z_direction_features is None:
                ...
                encoded_sentence = self.vln_bert.forward_txt(
                    ...
                    front_txt_embeds=front_txt_feats,
                )
            ...
            if 'rgb' in observations:
                ...
                batch_size = observations['rgb_0.0'].shape[0]
            ''' encoding rgb/depth at all directions ----------------------------- '''
            NUM_ANGLES = 120    # 120 angles 3 degrees each
            NUM_IMGS = 12
            NUM_CLASSES = 12    # 12 distances at each sector
            if 'rgb' in observations:
                depth_batch = torch.zeros_like(observations['depth']).repeat(NUM_IMGS, 1, 1, 1) # first dimension becomes 12*batch_size
                rgb_batch = torch.zeros_like(observations['rgb']).repeat(NUM_IMGS, 1, 1, 1)
            else:
                depth_batch = torch.zeros_like(observations['depth_0.0']).repeat(NUM_IMGS, 1, 1, 1) # first dimension becomes 12*batch_size
                rgb_batch = torch.zeros_like(observations['rgb_0.0']).repeat(NUM_IMGS, 1, 1, 1)
                
            sorted_keys = sorted([k for k in observations.keys() if 'depth' in k], 
                               key=self.get_angle)
            a_count = 0
            ...
            rgb_embed_reshape = rgb_embedding.reshape(
                batch_size, NUM_IMGS, self.config.CLIP_ENCODER.output_size, 1, 1)
            ...
            outputs = {
                'cand_rgb': cand_rgb,               # [K x 2048] / [K x 512]
                ...
                'pano_rgb': pano_rgb,               # B x 12 x 2048(512)
            ...
            if not already_dropout:
                rgb_fts = self.drop_env(rgb_fts)
            if self.config.add_depth_dropout:
                dep_fts = self.drop_env(dep_fts) 
            if z_img_features is None:
            ...
            if self.config.add_global_dropout:
                gmap_img_fts = self.drop_env(gmap_img_fts)
            outs = self.vln_bert.forward_navigation_with_local_branch(
                txt_embeds, txt_masks,
                gmap_vp_ids, gmap_step_ids,
                gmap_img_fts, gmap_pos_fts,
                gmap_masks, gmap_visited_masks, gmap_pair_dists,
                vp_img_embeds, vp_pos_fts, vp_masks, vp_nav_masks, vp_cand_vpids,
                front_txt_feats, front_vp_feats, front_gmap_feats
            )
            return outs
