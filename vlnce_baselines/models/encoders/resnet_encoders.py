...
class CLIPEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.config.CLIP_ENCODER.model == "ViT-B/32":
            self.model, _ = clip.load("ViT-B/32", device=device)
            self.return_before_proj = False
        elif "CLIP-H14" in self.config.CLIP_ENCODER.model:
            # for DUET-scalevln
            self.model, _, _ = open_clip.create_model_and_transforms(
                'ViT-H-14',
                pretrained=self.config.CLIP_ENCODER.model
            )
            self.return_before_proj = False
        elif "CLIP-ViT-B-16" in self.config.CLIP_ENCODER.model:
            # for MAGIC
            self.model, _, _ = open_clip.create_model_and_transforms(
                'ViT-B-16',
                pretrained=self.config.CLIP_ENCODER.model
            )
            self.return_before_proj = True
        elif "clip-vit-base-patch16" in self.config.CLIP_ENCODER.model:
            self.model, self.preprocess = clip.load(self.config.CLIP_ENCODER.model, device=device)
            self.return_before_proj = True
            self.clip_debug = True # for debug
        elif "vit_base_patch16_224" in self.config.CLIP_ENCODER.model:
            import timm
            from timm.data import resolve_data_config
            from timm.data.transforms_factory import create_transform
            self.model = timm.create_model(self.config.CLIP_ENCODER.model, pretrained=False, checkpoint_path=None, num_classes=0).to(device)
            state_dict = torch.load(self.config.CLIP_ENCODER.model_path, map_location=lambda storage, loc: storage)
    ...
    def forward(self, observations):
        ...
        if self.clip_debug:
            ...
            for img in observations["rgb"]:
                image = Image.fromarray(img.cpu().numpy())
                image = self.preprocess(image)
                images.append(image)
            images = torch.stack(images).to(device)
            # output = self.model.encode_image(images, return_before_proj=self.return_before_proj)
            output = self.model.forward_features(images)[:,0]
            
        else:
            rgb_observations = observations["rgb"].permute(0, 3, 1, 2)
            rgb_observations = self.rgb_transform(rgb_observations)                                                                                                             
            output = self.model.encode_image(rgb_observations.contiguous(), return_before_proj=self.return_before_proj)
