...
def construct_envs(
    ...
    dataset = make_dataset(config.task_config.dataset.type, do_shuffle=do_shuffle)
    scenes = config.task_config.dataset.content_scenes
    if "*" in config.task_config.dataset.content_scenes:
        scenes = dataset.get_scenes_to_load(config.task_config.dataset)
    logger.info(f"SPLIT: {config.task_config.dataset.split}, NUMBER OF SCENES: {len(scenes)}")
...
def construct_envs_auto_reset_false(
    config, env_class: Type[Union[Env, RLEnv]]
) -> VectorEnv:
