...
class BaseVLNCETrainer(BaseILTrainer):
    ...
    def collect_val_traj(self):
        ...
        if 'reverie' in self.config.base_task_config_path:
            gt_data = None
            with open(
                self.config.task_config.dataset.eval_ep_ids.format(split=split),
            ) as f:
    ...
    def eval(self):
        ...
        with TensorboardWriter(
            ...
            if os.path.isfile(self.config.eval.ckpt_path_dir):
                current_ckpt = self.config.eval.ckpt_path_dir
                ckpt_file_ind = 0
                if self.config.task_config.dataset.split == 'val_seen_unseen':
                    unseen_spl, unseen_sr, seen_spl, seen_sr = self._eval_checkpoint(
                ...
                while True:
                    ...
                    while current_ckpt is None:
                        current_ckpt = poll_checkpoint_folder(
                            ...
                            prev_ckpt_file_ind=ckpt_file_ind
                        )
                        time.sleep(2)  # sleep for 2 secs before polling again
                    if self.local_rank < 1:
                        logger.info(f"=======current_ckpt: {current_ckpt}=======")
                    prev_ckpt_ind += 1
                    ckpt_file_ind = self.get_ckpt_id(current_ckpt)
                    if self.config.task_config.dataset.split == 'val_seen_unseen':
                        unseen_spl, unseen_sr, seen_spl, seen_sr = self._eval_checkpoint(
                            checkpoint_path=current_ckpt,
                            writer=writer,
                        ...
                        if unseen_spl == 0 and unseen_sr == 0 and seen_spl == 0 and seen_sr == 0:
                            continue
                        
                        self.logger.info(f"=======Eval Current_ckpt: {current_ckpt}=======")
                        # update results
                        ...
                        for split in ['val_unseen', 'val_seen']:
                            split_spl = unseen_spl if split == 'val_unseen' else seen_spl
                            split_sr = unseen_sr if split == 'val_unseen' else seen_sr
                            eval_results = self.val_unseen_eval_results if split == 'val_unseen' else self.val_seen_eval_results
                            
                            if split_spl > eval_results['best_spl']:
                                eval_results['best_spl'] = split_spl
                                eval_results[f'best_{split}_spl_index'] = ckpt_file_ind
                            ...
                            if split_sr > eval_results['best_sr']:
                                eval_results['best_sr'] = split_sr
                                eval_results['best_sr_index'] = ckpt_file_ind
                            ...
                            if split_spl + split_sr > eval_results['best_spl_sr']:
                                eval_results['best_spl_sr'] = split_spl + split_sr
                                eval_results['best_spl_sr_index'] = ckpt_file_ind
                                # copy the current ckpt file to best_spl_sr
                                shutil.copy(current_ckpt, os.path.join(self.config.EVAL_CKPT_PATH_DIR, 'ckpts', f'{split}_best_spl_sr.pth'))
                                eval_results['best_spl_sr_spl'] = split_spl
                                eval_results['best_spl_sr_sr'] = split_sr
                            
                            if self.config.EVAL.auto_remove:
                                if ckpt_file_ind != eval_results['best_spl_index'] and ckpt_file_ind != eval_results['best_sr_index']:
                                    # remove the ckpt_file
                                    os.remove(current_ckpt)
                            
                            # update the eval log
                            step_file_ind = -1
                            self.logger.info(f"Current {split} SPL: {split_spl:.4f} at the epoch {ckpt_file_ind} , iteration {step_file_ind}")
                            self.logger.info(f"Current {split} SR: {split_sr:.4f} at the epoch {ckpt_file_ind} , iteration {step_file_ind}")
                            self.logger.info(f"Current {split} SPL and SR: {split_spl+split_sr:.4f} at the epoch {ckpt_file_ind} , iteration {step_file_ind}")
                            
                            self.logger.info(f"Best {split} SPL: {eval_results['best_spl']:.4f} at index {eval_results['best_spl_index']}")
                            self.logger.info(f"Best {split} SR: {eval_results['best_sr']:.4f} at index {eval_results['best_sr_index']}")
                            self.logger.info(f"Best {split} SPL and SR: {eval_results['best_spl_sr']:.4f} at index {eval_results['best_spl_sr_index']} . SPL: {eval_results['best_spl_sr_spl']:.4f} , SR: {eval_results['best_spl_sr_sr']:.4f}")
                        ...
                        # update the eval log
                        step_file_ind = -1
                        self.logger.info(f"Current {self.config.EVAL.SPLIT} SPL: {spl:.4f} at the epoch {ckpt_file_ind} , iteration {step_file_ind}")
                        self.logger.info(f"Current {self.config.EVAL.SPLIT} SR: {sr:.4f} at the epoch {ckpt_file_ind} , iteration {step_file_ind}")
                        self.logger.info(f"Current {self.config.EVAL.SPLIT} SPL and SR: {spl+sr:.4f} at the epoch {ckpt_file_ind} , iteration {step_file_ind}")
                        
                        self.logger.info(f"Best {self.config.EVAL.SPLIT} SPL: {self.eval_results['best_spl']:.4f} at index {self.eval_results['best_spl_index']}")
                        self.logger.info(f"Best {self.config.EVAL.SPLIT} SR: {self.eval_results['best_sr']:.4f} at index {self.eval_results['best_sr_index']}")
                        self.logger.info(f"Best {self.config.EVAL.SPLIT} SPL and SR: {self.eval_results['best_spl_sr']:.4f} at index {self.eval_results['best_spl_sr_index']} . SPL: {self.eval_results['best_spl_sr_spl']:.4f} , SR: {self.eval_results['best_spl_sr_sr']:.4f}")
