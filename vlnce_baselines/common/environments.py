...
class VLNCEDaggerEnv(habitat.RLEnv):
    def __init__(self, config, dataset: Optional[Dataset] = None):
    ...
    def multi_pointgoal_step_control(self, cfg, pointgoal_net, path, vis_info, max_pg_steps, obs_transforms, device):
        for vp, vp_pos in path: #path[::-1]:
            self.pointgoal_step_control(cfg, pointgoal_net, vp_pos, vis_info, max_pg_steps, obs_transforms, device)
    ...
    def pointgoal_step_control(self, cfg, pointgoal_net, ghost_pos, vis_info, max_pg_steps, obs_transforms, device):
        action_shape = (1, )
        action_type = torch.long
        # Init parameters
        test_recurrent_hidden_states = torch.zeros(
            1,
            pointgoal_net.net.num_recurrent_layers,
            cfg.rl.ppo.hidden_size,
            device=device,
        )
        ...
        output_stop_flag = False
        for step in range(max_pg_steps):
            # compute the pointgoal
            agent_state = self._env.sim.get_agent_state()
            agent_position = agent_state.position
            rotation_world_agent = agent_state.rotation

            goal_position = ghost_pos
            pointgoal = self._compute_pointgoal(agent_position, rotation_world_agent, goal_position)
            ...
            if step_data[0] == 0:
                # reach the local goal
                output_stop_flag = True
                break
            
            observations = self._env.step(step_data[0])
            if self.video_option:
                info = self.get_info(observations)
                self.video_frames.append(
                    navigator_video_frame(
            ...
            self._env._task.measurements.update_measures(
                episode=self._env.current_episode, action=step_data, task=self._env.task 
            )
            

            not_done_masks = torch.ones_like(not_done_masks)
        
        return output_stop_flag
    ...
    def step(self, action, vis_info, *args, **kwargs):
        ...
        if act == 4: # high to low
        ...
        elif act == 5: # pointgoal
            cfg = action['cfg']
            pointgoal_net = action['pointgoal_net']
            max_pg_steps = action['max_pg_steps']
            obs_transforms = action['obs_transforms']
            ...
            if action['back_path'] is None:
                ...
                self.multi_pointgoal_step_control(cfg, pointgoal_net, action['back_path'], vis_info, max_pg_steps, obs_transforms, device)
            
            # 2. pointgoal to ghost node
            output_stop_flag = self.pointgoal_step_control(cfg, pointgoal_net, action['ghost_pos'], vis_info, max_pg_steps, obs_transforms, device)
            agent_state = self._env.sim.get_agent_state()
            observations = self.get_observation_at(agent_state.position, agent_state.rotation)
        ...
        if act == 5:
            info['output_stop_flag'] = output_stop_flag
